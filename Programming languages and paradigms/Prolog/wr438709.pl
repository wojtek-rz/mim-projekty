...
findPathBfs(Program, N, Visited, Queue, VisitedOut, PathOut) :-
    (   is_queue_empty(Queue) -> % if queue is empty
        ...
        (   
            length(Visited, VisitedLength),
            writeln(VisitedLength),
...
step(program(_, _, Instructions), state(Vars, Arrs, InstrPointers), 
    Id, ResState) :-
    nth0(Id, InstrPointers, InstrPointer),
    nth1(InstrPointer, Instructions, Instr),
...
is_queue_empty(Front - Back) :- Front == Back.

queue_add(X-[E|Z], E, X-Z).

queue_pop([E|X]-Z, E, X-Z).
