...
data TopDef' a
    = ClsDef a Ident [ClsDefItem' a]
    | FnDef a (Type' a) Ident [Arg' a] (Block' a)
  deriving (C.Eq, C.Ord, C.Show, C.Read, C.Functor, C.Foldable, C.Traversable)

type ClsDefItem = ClsDefItem' BNFC'Position
data ClsDefItem' a = AttrDef a (Type' a) Ident
  deriving (C.Eq, C.Ord, C.Show, C.Read, C.Functor, C.Foldable, C.Traversable)
...
data LValue' a
    = LVar a Ident
    | LArr a (LValue' a) (Expr' a)
    | LAttr a (LValue' a) Ident
  deriving (C.Eq, C.Ord, C.Show, C.Read, C.Functor, C.Foldable, C.Traversable)
...
instance HasPosition ClsDefItem where
  hasPosition = \case
    AttrDef p _ _ -> p
